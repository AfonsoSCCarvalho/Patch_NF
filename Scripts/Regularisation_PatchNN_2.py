...
def Regul_PatchNN_2(Z, X0, MS, HS, Y_M, Y_H_Ut, UR, sigmaM, sigmaH, sigmaM_other, B, subsamp, U, mu, lam=0.000000025,
    ...
    # Initialize a variable to keep track of the previous value of Obj_F1
    prev_Obj_F1 = float('inf')
    MS_compare = torch.mean(MS, dim=2).unsqueeze(0)
    ...
    for iter in range(niter):
        ...
        for j in range(Z.shape[1]):  # Assuming Z.shape[1] is the number of components
            ...
            min_value_Z = torch.min(Z_3d_specter_first[:, j, :, :])
            max_value_Z = torch.max(Z_3d_specter_first[:, j, :, :])

            Z_3d_normalized = (Z_3d_component - min_value_Z) * (1 - 0) / (max_value_Z - min_value_Z)

            # Create patches for both Z and MS for comparison
            fake_data_1, fake_data_2 = input_im2pat(Z_3d_normalized[:, j, :, :].unsqueeze(0), MS_compare.unsqueeze(0), batch_size=0, high_frequency_selection=False)
            pred_inv_1, log_det_inv_1 = model(fake_data_1, rev=True)
            pred_inv_2, log_det_inv_2 = model(fake_data_2, rev=True)

            reg_term = torch.mean(torch.sum(pred_inv_1**2, dim=1) / 2) - torch.mean(log_det_inv_1)  + torch.mean(torch.sum((pred_inv_1 - pred_inv_2)**2, dim=1) / 2)


            reg_terms[j].append(reg_term.item())
            reg += get_reg_coeff[j] * reg_term
