...
def FinalResults_reg(HS, HS_np, MS, MS_np, Img_reconstructed_adam, Img_reconstructed_reg, X0, X0_np, target_shape):

    quality_indices_torch(Img_reconstructed_reg, X0)


    Img_reconstructed_adam_np = Img_reconstructed_adam.detach().cpu().numpy()
    Img_reconstructed_np = Img_reconstructed_reg.detach().cpu().numpy()
    ...
    temp_show_Z = normColor(temp_show_Z)
    axs[1, 1].imshow(temp_show_Z)
    axs[1, 1].set_title("Img reconstructed REG")
    axs[1, 1].axis('off')
